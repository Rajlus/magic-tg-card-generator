...
def validate_imports() -> tuple[bool, Any | None, list[str]]:
...
def validate_constants(generator_class) -> tuple[bool, dict[str, Any], list[str]]:
    ...
    expected_constants = {
        ...
        "INVALID_FILENAME_CHARS": (
            None,
            list,
        ),  # List content will be validated separately
    }
        ...
        try:
            if hasattr(generator_class, const_name):
                ...
                if not isinstance(actual_value, expected_type):
                    errors.append(
                        f"Constant {const_name} has wrong type. Expected {expected_type.__name__}, got {type(actual_value).__name__}"
                    )
                    all_passed = False
                    print(
                        f"   ❌ {const_name}: Wrong type ({type(actual_value).__name__})"
                    )
                    continue
                ...
                if expected_value is not None and actual_value != expected_value:
                    errors.append(
                        f"Constant {const_name} has wrong value. Expected {expected_value}, got {actual_value}"
                    )
                    all_passed = False
                ...
                if const_name == "INVALID_FILENAME_CHARS":
                    ...
                    if not all(char in actual_value for char in expected_chars):
                        missing_chars = [
                            char for char in expected_chars if char not in actual_value
                        ]
                        errors.append(
                            f"INVALID_FILENAME_CHARS missing expected characters: {missing_chars}"
                        )
                        all_passed = False
                        print(f"   ❌ {const_name}: Missing characters {missing_chars}")
                        continue

                    constants_info[const_name]["expected_chars_count"] = len(
                        expected_chars
                    )
...
def validate_class_instantiation(generator_class) -> tuple[bool, Any | None, list[str]]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            generator = generator_class(
                output_dir=str(output_dir), images_dir=str(images_dir)
            )
            ...
            generator_with_model = generator_class(
                output_dir=str(output_dir), images_dir=str(images_dir), api_model="sdxl"
            )
...
def validate_basic_functionality(generator) -> tuple[bool, dict[str, Any], list[str]]:
    ...
    try:
        ...
        for mana_cost, expected_colors in test_mana_costs:
            try:
                ...
                if result != expected_colors:
                    errors.append(
                        f"_extract_colors({mana_cost}) returned {result}, expected {expected_colors}"
                    )
                    colors_test_passed = False
                    print(
                        f"   ❌ _extract_colors({mana_cost}): got {result}, expected {expected_colors}"
                    )
                else:
        ...
        try:
            ...
            for prompt in test_prompts:
                try:
                    ...
                    if missing_fields:
                        errors.append(
                            f"generate_from_prompt('{prompt}') missing fields: {missing_fields}"
                        )
                        prompt_test_passed = False
                        print(
                            f"   ❌ generate_from_prompt('{prompt}'): missing {missing_fields}"
                        )
                    else:
                        print(
                            f"   ✅ generate_from_prompt('{prompt}'): generated '{result['name']}'"
                        )
        ...
        try:
            ...
            functionality_results["filename_sanitization"] = {
                "passed": False,
                "error": str(e),
            }
            all_passed = False
...
def run_existing_tests() -> tuple[bool, str]:
    ...
    try:
        ...
        test_files = ["test_core.py", "test_models.py"]
...
def main():
    ...
    constants_success, constants_info, constants_errors = validate_constants(
        generator_class
    )
    ...
    ) = validate_class_instantiation(generator_class)
    ...
    if instantiation_success and generator_instance:
        ...
        ) = validate_basic_functionality(generator_instance)
        ...
        results["warnings"].append(
            "Basic functionality tests skipped due to instantiation failure"
        )
...
def print_final_report(results: dict[str, Any]):
    ...
    if all_critical_passed:
        if results["basic_functionality"]:
            ...
            print(
                "✅ OVERALL STATUS: CRITICAL TESTS PASSED (Some basic functionality issues)"
            )
    else:
