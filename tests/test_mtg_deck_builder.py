...
class TestCardGeneratorWorker:
    ...
    def test_card_generator_worker_initialization(self, qapp):
        ...
        worker = CardGeneratorWorker(
            "Lightning Bolt Dragon", "A fierce dragon creature", "creature"
        )
...
class TestThemeConfigTab:
    ...
    def test_load_themes(self, theme_tab):
        ...
        with patch("mtg_deck_builder.Path.exists", return_value=True), patch(
            "mtg_deck_builder.Path.glob"
        ) as mock_glob:
...
class TestMTGDeckBuilder:
    ...
    def deck_builder(self, qapp):
        ...
        with patch("mtg_deck_builder.QSettings"), patch.object(
            MTGDeckBuilder, "load_card_database"
    ...
    def test_save_deck(self, deck_builder):
        ...
        try:
            with patch(
                "mtg_deck_builder.QFileDialog.getSaveFileName",
    ...
    def test_load_deck(self, deck_builder):
        ...
        try:
            with patch(
                "mtg_deck_builder.QFileDialog.getOpenFileName",
                return_value=(temp_path, "JSON Files (*.json)"),
            ):
    ...
    def test_filter_cards_by_mana_cost(self, deck_builder):
        ...
        one_cmc_cards = [
            card for card in deck_builder.filtered_cards if card.get("cmc") == 1
        ]
    ...
    def test_search_functionality(self, deck_builder):
        ...
        lightning_cards = [
            card for card in deck_builder.filtered_cards if "Lightning" in card["name"]
        ]
...
class TestIntegration:
    ...
    def app_with_mocks(self, qapp):
        ...
        with patch("mtg_deck_builder.QSettings"), patch(
