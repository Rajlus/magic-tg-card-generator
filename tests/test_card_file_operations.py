    ...
    def test_deck_directory_management(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert (
                    self.file_ops.get_rendered_cards_directory()
                    == deck_dir / self.file_ops.RENDERED_CARDS_SUBDIR
                )
                assert (
                    self.file_ops.get_artwork_directory()
                    == deck_dir / self.file_ops.ARTWORK_SUBDIR
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(csv_file, "w", newline="") as f:
                writer = csv.DictWriter(
                    f, fieldnames=self.file_ops.CSV_FIELDNAMES, delimiter=";"
                )
                writer.writeheader()
