...
class TestFilteringAndSearch:
    ...
    def test_search_by_name(self, sample_card_database):
        ...
        exact_results = [
            card
            for card in sample_card_database
        ...
        # Test partial match
        partial_results = [
            card for card in sample_card_database if "lightning" in card["name"].lower()
        ]
        ...
        case_results = [
            card for card in sample_card_database if "LIGHTNING" in card["name"].upper()
        ]
    ...
    def test_search_by_oracle_text(self, sample_card_database):
        ...
        mana_results = [
            card
            for card in sample_card_database
            if "add" in card["oracle_text"].lower()
            and "mana" in card["oracle_text"].lower()
        ]
        assert len(mana_results) == 1  # Only Black Lotus has both "add" and "mana"
        assert mana_results[0]["name"] == "Black Lotus"

    def test_filter_by_color(self, sample_card_database):
        """Test filtering cards by color."""
        # Filter red cards
        red_cards = [card for card in sample_card_database if "R" in card["colors"]]
        assert len(red_cards) == 2
        assert all("R" in card["colors"] for card in red_cards)

        # Filter green cards
        green_cards = [card for card in sample_card_database if "G" in card["colors"]]
        assert len(green_cards) == 3

        # Filter colorless cards
        colorless_cards = [card for card in sample_card_database if not card["colors"]]
        ...
        multicolor_cards = [
            card for card in sample_card_database if len(card["colors"]) > 1
        ]
    ...
    def test_filter_by_color_identity(self, sample_card_database):
        ...
        red_identity = [
            card for card in sample_card_database if "R" in card["color_identity"]
        ]
        assert len(red_identity) == 2

        # Filter by green color identity
        green_identity = [
            card for card in sample_card_database if "G" in card["color_identity"]
        ]
        assert len(green_identity) == 3

        # Filter colorless identity
        colorless_identity = [
            card for card in sample_card_database if not card["color_identity"]
        ]
    ...
    def test_filter_by_card_type(self, sample_card_database):
        ...
        # Filter instants
        instants = [
            card for card in sample_card_database if "Instant" in card["type_line"]
        ]
        ...
        # Filter creatures
        creatures = [
            card for card in sample_card_database if "Creature" in card["type_line"]
        ]
        assert len(creatures) == 2

        # Filter artifacts
        artifacts = [
            card for card in sample_card_database if "Artifact" in card["type_line"]
        ]
        assert len(artifacts) == 2

        # Filter legendary cards
        legendary = [
            card for card in sample_card_database if "Legendary" in card["type_line"]
        ]
    ...
    def test_filter_by_rarity(self, sample_card_database):
        ...
        # Filter common cards
        commons = [card for card in sample_card_database if card["rarity"] == "common"]
        assert len(commons) == 5

        # Filter uncommon cards
        uncommons = [
            card for card in sample_card_database if card["rarity"] == "uncommon"
        ]
        assert len(uncommons) == 1
        assert uncommons[0]["name"] == "Sol Ring"

        # Filter mythic cards
        mythics = [card for card in sample_card_database if card["rarity"] == "mythic"]
        assert len(mythics) == 2

    def test_filter_by_set(self, sample_card_database):
        """Test filtering cards by set."""
        # Filter M21 cards
        m21_cards = [card for card in sample_card_database if card["set"] == "M21"]
        assert len(m21_cards) == 5

        # Filter CMD cards
        cmd_cards = [card for card in sample_card_database if card["set"] == "CMD"]
        assert len(cmd_cards) == 1
        assert cmd_cards[0]["name"] == "Sol Ring"

        # Filter LEA cards
        lea_cards = [card for card in sample_card_database if card["set"] == "LEA"]
    ...
    def test_filter_by_power_toughness(self, sample_card_database):
        ...
        power_2 = [card for card in sample_card_database if card.get("power") == 2]
        assert len(power_2) == 1
        assert power_2[0]["name"] == "Grizzly Bears"

        # Filter creatures with toughness 2
        toughness_2 = [
            card for card in sample_card_database if card.get("toughness") == 2
        ]
    ...
    def test_advanced_text_search(self, sample_card_database):
        ...
        def search_text(cards, query):
            ...
            return [
                ...
                for card in cards
                if all(
                    word in (card["name"] + " " + card["oracle_text"]).lower()
                    for word in words
                )
            ]
    ...
    def test_filter_performance(self, sample_card_database):
        ...
        assert (
            len(red_cards)
            == len(sample_card_database) * 1000 // len(sample_card_database) * 2
        )
        assert (
            len(instants)
            == len(sample_card_database) * 1000 // len(sample_card_database) * 4
        )
    ...
    def test_filter_caching(self, sample_card_database):
        ...
        def red_filter(card):
            return "R" in card["colors"]

        def instant_filter(card):
            return "Instant" in card["type_line"]

        # First call - should cache result
        red_cards_1 = cached_filter(sample_card_database, red_filter, "red_cards")
        instant_cards_1 = cached_filter(
            sample_card_database, instant_filter, "instant_cards"
        )

        # Second call - should use cache
        red_cards_2 = cached_filter(sample_card_database, red_filter, "red_cards")
        instant_cards_2 = cached_filter(
            sample_card_database, instant_filter, "instant_cards"
        )
    ...
    def test_sorting_functionality(self, sample_card_database):
        ...
        sorted_by_rarity = sorted(
            sample_card_database, key=lambda x: rarity_order.get(x["rarity"], 0)
        )
        assert sorted_by_rarity[0]["rarity"] == "common"
        assert sorted_by_rarity[-1]["rarity"] == "mythic"

        # Sort by multiple criteria (CMC, then name)
        sorted_multi = sorted(sample_card_database, key=lambda x: (x["cmc"], x["name"]))
