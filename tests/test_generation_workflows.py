...
class TestGenerationWorkflows:
        ...
        for template_type, template_data in templates.items():
            with patch('subprocess.run') as mock_subprocess:
                ...
                cmd = [
                    ...
                    "--name", f"Test {template_type.title()}"
                ]
    ...
    def test_generation_queue_management(self):
        ...
        generation_queue.sort(key=lambda x: priority_order.get(x["request"]["priority"], 0), reverse=True)
        ...
        with patch('subprocess.run') as mock_subprocess:
            ...
            for item in generation_queue[:2]:  # Process first 2 items
                ...
                # Simulate generation
                result = subprocess.run(["python", "generate_card.py"], capture_output=True, text=True)
                if result.returncode == 0:
    ...
    def test_generation_performance_monitoring(self):
        ...
        if generation_stats["generation_times"]:
            generation_stats["average_generation_time"] = sum(generation_stats["generation_times"]) / len(generation_stats["generation_times"])
