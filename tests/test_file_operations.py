...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            ...
            lands = [card for card in deck_cards if "Land" in card.get("type_line", "") or card["mana_cost"] == ""]
            non_lands = [card for card in deck_cards if "Land" not in card.get("type_line", "") and card["mana_cost"] != ""]
    ...
    def test_export_deck_mtga_format(self):
        ...
        deck_cards = [
            {"name": "Lightning Bolt", "quantity": 4, "set": "M21", "collector_number": "160"},
            {"name": "Shock", "quantity": 4, "set": "M21", "collector_number": "159"},
            {"name": "Mountain", "quantity": 20, "set": "ZNR", "collector_number": "274"}
        ]
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            ...
            for card in deck_cards:
                ...
                f.write(f"{card['quantity']} {card['name']} ({set_code}) {collector_num}\n")
...
"""
        ...
        try:
            ...
            # Verify parsing
            assert len(cards) > 0
            assert any(card["name"] == "Lightning Bolt" and card["quantity"] == 4 for card in cards)
            assert any(card["name"] == "Mountain" and card["quantity"] == 20 for card in cards)
    ...
    def test_deck_statistics_export(self):
        ...
        deck_cards = [
            {"name": "Lightning Bolt", "cmc": 1, "type_line": "Instant", "colors": ["R"]},
            {"name": "Shock", "cmc": 1, "type_line": "Instant", "colors": ["R"]},
            {"name": "Grizzly Bears", "cmc": 2, "type_line": "Creature — Bear", "colors": ["G"]},
