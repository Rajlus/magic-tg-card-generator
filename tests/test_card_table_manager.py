...
from PyQt6.QtWidgets import QApplication, QTableWidget, QComboBox, QLineEdit, QLabel
...
class MockCard:
    ...
    def __init__(self, id=1, name="Test Card", cost="2R", type="Creature", 
                 power="2", toughness="2", text="Test card text", 
                 rarity="common", art="Test art description", status="pending"):
        self.id = id
        self.name = name
        self.cost = cost
        self.type = type
...
class TestCardTableManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cards = [
            MockCard(1, "Lightning Bolt", "R", "Instant", None, None, "Deal 3 damage", "common", "Red lightning"),
            MockCard(2, "Grizzly Bears", "1G", "Creature — Bear", "2", "2", "A bear", "common", "Brown bear"),
            MockCard(3, "Black Lotus", "0", "Artifact", None, None, "Add 3 mana", "mythic", "Black flower")
        ]
    ...
    def test_required_methods_exist(self):
        ...
        for expected_name, actual_method in required_methods.items():
            has_method = hasattr(self.manager, actual_method) and callable(getattr(self.manager, actual_method))
    ...
    def test_refresh_table(self):
        ...
        first_row = 0
        self.assertEqual(self.table.item(first_row, CardTableManager.COLUMN_ID).text(), "1")
        self.assertEqual(self.table.item(first_row, CardTableManager.COLUMN_NAME).text(), "Lightning Bolt")
        self.assertEqual(self.table.item(first_row, CardTableManager.COLUMN_COST).text(), "R")
        self.assertEqual(self.table.item(first_row, CardTableManager.COLUMN_TYPE).text(), "Instant")
    ...
    def test_apply_filter_type(self):
        ...
        for row in range(self.table.rowCount()):
            if not self.table.isRowHidden(row):
                ...
                self.assertIn("Instant", self.table.item(row, CardTableManager.COLUMN_TYPE).text())
    ...
    def test_search_filter(self):
        ...
        for row in range(self.table.rowCount()):
            if not self.table.isRowHidden(row):
                visible_count += 1
                self.assertIn("Lightning Bolt", self.table.item(row, CardTableManager.COLUMN_NAME).text())
    ...
    def test_card_addition_removal(self):
        ...
        # Add a card
        new_card = MockCard(4, "Test Addition", "2U", "Creature", "1", "1", "Test", "common", "Test art")
